...
本示例演示如何使用 **Blackfly USB3 相机（Spinnaker SDK）** 进行图像采集，并在按下 **空格键** 时将当前图像保存到指定目录，按下**删除键**即可实现删除拍的上一张图片。支持自定义保存目录与图片组命名。
记得要以**管理员权限运行**，linux直接**sudo**运行
